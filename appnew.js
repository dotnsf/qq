...
app.post( '/api/answer', function( req, res ){
  ...
  if( doc ){
    ...
    doc.created = ts;
    doc.updated = ts;
    client.postDocument( { db: settings.db_name, document: doc } ).then( function( result ){
...
app.get( '/api/answer/:answer_id', function( req, res ){
  ...
  if( id ){
    var param = { db: settings.db_name, docId: id };
...
app.get( '/api/answers/:question_id', function( req, res ){
  ...
  if( id ){
    var param = { db: settings.db_name, selector: { '$and': [ { question_id: { '$eq': id } }, { type: { '$eq': 'answer' } } ] }, fields: [ '_id', '_rev', 'user_id', 'question_id', 'answers', 'type', 'created', 'updated' ] };
...
app.get( '/api/questions/:user_id', function( req, res ){
  ...
  if( id ){
    var param = { db: settings.db_name, selector: { '$and': [ { user_id: { '$eq': id } }, { type: { '$eq': 'question' } } ] }, fields: [ '_id', '_rev', 'user_id', 'label', 'questions', 'type', 'created', 'updated' ] };
    client.postFind( param ).then( function( docs ){
      if( docs && docs.result && docs.result.docs ){
  ...
  if( id ){
    ...
    client.getDocument( param ).then( function( doc ){
      if( doc && doc.result && doc.result.type && doc.result.type == 'answer' ){
        ...
        client.postDocument( { db: settings.db_name, document: newdoc } ).then( function( result ){
          res.write( JSON.stringify( { status: true, result: result.result } ) );
          res.end();
        }).catch( function( err ){
          res.status( 400 );
